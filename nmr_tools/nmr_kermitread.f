...
C call user routine
       call nmr_read_image( map_array, status )
...
*+ nmr_read_image
       ...
       subroutine nmr_read_image(map_array,status)
C      -------------------------------------------
C
...
C find input style
       ...
       ny = nx
       call io_geti('Y/V-dimension : ','*',ny,status)
       call io_geti('Number-of-frames : ','1',nf,status)
       if (nf*nx*ny.gt.bsize) then
...
C read each image frame
       ...
       do n=1,nf
         ...
         do j=1,nx*ny
           ...
           buffer(m) = 256*buffer(m) + ichar(next_byte)
           i = i + 1
           read(iunit,REC=i) next_byte
           buffer(m) = 256*buffer(m) + ichar(next_byte)
           i = i + 1
...
C place data into map catalogue
       ...
       do n=1,nf
         ...
         if (nf.gt.1) then
           ...
           string = source(1:chr_lenb(source))//'-'//val(1:ls)
         else
           string = source
         endif
