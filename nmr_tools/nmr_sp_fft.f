C NMR FFT: take fft of data allowing for interpolation of imaginary data.
C
       implicit    NONE
C
C
...
C .. interolate imaginary data
         call nmr_interp( yi, ndata, status )
       endif
       if (narr.eq.2) then
...
C
C
       subroutine nmr_interp( y, ndata, s )
C      -----------------------------------------
C
C Perform simple mid-point interpolation
C
C Updated:
C   data array
       real*4    y(*)
