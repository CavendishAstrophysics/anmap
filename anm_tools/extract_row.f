...
C read input map: imap is the catalogue identifier
       call do_extract_row(map_array,status)

C check status value and report an error to the user
       call cmd_err(status,'EXTRACT-ROW','Failed ')
...
*+ do_extract_row
       ...
       subroutine do_extract_row(map_array,status)
C      -------------------------------------------
C
C Extract a specifiewd row from the image
C
C Given:
C   map data
       real*4        map_array(*)
...
C   error status word
       integer       status
C
C A specified row is extracted from the image and the results written
...
C read in the map
       ...
       if (status.ne.0) then
         call cmd_err(status,'EXTRACT-ROW',' ')
         goto 999
       end if
       call enminirt(minirt,status)

C find user defined parameters
       call io_geti('Row to extract (V) : ','0',iv,status)
...
C write this out to the results file
     ...
     *                 'Error accessing results file')
       ...
       write (iout,*)'%ndata ',minirt(5)
       write (iout,*)'%ncols 2'
       write (iout,*)'%title Extract-Column ',iv
       do i = 1, minirt(5)
         write (iout,*) x(i), y(i)
       end do
       close (iout)
