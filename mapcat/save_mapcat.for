...
C find pointers
       ...
       call stack_util_enqdat(is_redt,data_in_core,status)
...
C .. if access mode is DIRECT then find space
       ...
       else if (chr_cmatch(access,'SEQUENTIAL')) then
C .. set mode to be sequential
         call mapcat_mapopen(imap,'WRITE',iunit,status)
         call stack_access(imap,'SEQUENTIAL','MAP',0,iunit,status)
...
C .. IMAP data not in core allocate space and read data
     ...
     *                       minirt(5)*minirt(6),is_mapo,status)
           ...
           call mapcat_mapopen(imap,'READ',iunit,status)
           call rdmap(iunit,1,map_array(ip_mapo),status)
           call mapcat_mapclose(imap,status)
...
C allocate space for this map
     ...
     *                     is_map,status)
         call stack_util_enqdat(is_map,data_in_core,status)
         call stack_enqpnt(is_map,ip_map,status)
C .. read data into core if required
         if (data_in_core.eq.false) then
           call mapcat_mapopen(imap,'READ',iunit,status)
           call rdmap(iunit,1,map_array(ip_map),status)
           call mapcat_mapclose(imap,status)
         end if
         call stack_setdat(is_map,true,status)
       else if (chr_cmatch(access,'SEQUENTIAL')) then
         call stack_enqmap(imap,is_map,status)
         if (is_map.gt.0) then
           call stack_access(imap,'READ','MAP',
     *                       current_minirt(5)*current_minirt(6),
     *                       is_map,status)
...
C decode mode and take appropriate action
       if (mode.eq.mode_direct) then
         ...
         ip_map=ipm
         call rdrow(iunit,1,iv,map_array(ip_map),status)
...
C Prompt for filename description
       ...
       if (len_fd.eq.0) then
         file_description=
     *        dir_char_left//default_dir(1:len_d)//dir_char_right
...
*+ stack_util_enqdat
       ...
       subroutine stack_util_enqdat(is,data_code,status)
C      --------------------------------------------
C
...
*-
       ...
       if (status.ne.0) then
         call mapcat_err(status,'stack_util_enqdat',' ')
       else
