...
*+ do_reproj2
      ...
      subroutine do_reproj2(map_array,s)
C     ------------------------------------
C
...
C 16/7/98 SEGH - output map uv calc from user-supplied input map uv reg,
C                by invoking new routine repruv.f
C 31/7/98 SEGH - now calls map_alloc_out_s to deal correctly with
C                output map which has offset centre and different uv .      
C
*-
...
C local variables
      integer             iuv(4),iuv2(4),iproj2,iprec, i
...
C .. read in skew angle in HMS
      ...
      call plot_getuv ('UV-range of input map wanted: ', '*', iuv, s )
...
C     Reproject
C     First estimate equivalent uv range required for output map...
      call repruv(  iuv, iuv2, iproj2, iprec, usamp2, skew2,
     *              ramap2, decmap2, refdat2, epoch2, s )
C     Allocate output map
      call map_alloc_out_s( iuv2,'DIRECT',imapo,ip_mapo,s)
      call io_wrout('.. working')
C     Calculate output array 
      call reproj(  map_array(ip_map), map_array(ip_mapo), iuv2,
     *              iproj2, iprec, usamp2, skew2, ramap2, decmap2,
     *              refdat2, epoch2, degrid_type, s )
      if (s.ne.0) goto 9999

C     Set up redtape for output map
      call stredt( iuv2, 3, s )
...
C projection information in redtape
     ...
     *string='Reprojected to tangent plane '
      ...
      string=' '
      if (iproj2.eq.1) write (string,510) usamp2
510   format ('Aerial coords  ',f4.1,'"/gp')
      if (iproj2.eq.2) write (string,520) usamp2
520   format ('Sky coords     ',f4.1,'"/gp')
      if (iproj2.eq.3) write (string,530) usamp2
