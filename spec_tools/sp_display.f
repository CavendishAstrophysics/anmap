...
C   device size specification
       real*4     device_size(2)
C   data
       real*4     x(max_ndata), y(max_ndata)
       real*4     x1(max_ndata), y1(max_ndata)
C   file and directory name(s) and strings
       character  file*100, dir*100, string*10, text*80
...
C   option to control offset of multiple plots
       logical    do_offset, do_offset_list
       real*4     offset, offset_extra
...
C   character attributes
       ...
       real*4     character_size, symbol_size, yl_off
C   line options
...
C open plotting device
     ...
     *                 multi_xy(1), multi_xy(2) )
       ...
       call cmd_itemset('device_size', set, status )
       if (set) then
         print *,'.. setting device_size'
         call cmd_itemr('device_size', 2, device_size, status )
         call pgpaper(device_size(1),device_size(2))
       endif
...
C .. character size and font
       ...
       call cmd_itemr( 'symbol_size', 1, symbol_size, status )
...
C .. frame options
       ...
       call cmd_itemset( 'plot_xaxis', set, status )
       if (set) ytype = ' '
...
C draw box
     ...
     *                   title(1:chr_lenb(title)) )
           call cmd_itemr( 'yl_off', 1, yl_off, status)
           call pgmtext( 'L', yl_off, 0.5, 0.5, 
...
C .. re-draw frame and label
     ...
     *           data_max(1).ne.0.0) then
           ...
           if (.not.multi_plot) then
             string = ' '
             if (n1.le.9) then
               write(string,'(A,I1)') 'offset',n1
             else
               write(string,'(A,I2)') 'offset',n1
             endif
             call cmd_itemset( string, do_offset_list, status)
             if (do_offset_list) then
               call cmd_itemr( string, 1, offset_extra, status )
               do n=1,ndata
                 y(n) = y(n) + offset_extra
               enddo
             endif
           endif
