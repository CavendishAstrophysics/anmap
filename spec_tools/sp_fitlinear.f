C SPECTRUM: FITLINEAR
C -------------------
C
C Fit a linear model to a data set
C
C Input:
C   - file for fitting
C Output:
...
C data arrays passed in common
       ...
       real*8     x8(max_size), y8(max_size)
       common    /fitdat/ x8, y8
C
C Output unit
       integer    iout
...
C sort out options
       fit(1) = 0.0
       fit(2) = (y8(ndata)-y8(1))/(x_max-x_min)
       nvals = 2
...
C construct output spectrum as a fit to these data
       ndata = min(1000,10*ndata)
       step = (1.1*x_max-x_min)/float(ndata-1)
