...
{clear} {
	...
	set ce [lindex $args 0] ; iclPriv read $ce
	foreach n {access unit open} {set iclRec($n) 0}
	iclPriv write $ce
	return {}
	}
{initialise} {
...
#
# Procedures to provide editing of catalogue entries
#
proc imcat_edit { ceFrom ceTo command } {
   global iclRec iclPriv
   for {set ce $ceFrom} {$ce <= $ceTo} {incr ce} {
	iclPriv read $ce
	uplevel 1 $command
...
# set directory name
proc imcat_editSetDir { from to dir } {
   global iclRec
   imcat_edit $from $to {set iclRec(file) $dir/[file tail $iclRec(file)]}
}

# change directory name
proc imcat_editChangeDir { from to dirFrom dirTo } {
   global iclRec
   imcat_edit $from $to {
	regsub -all $dirFrom $iclRec(file) $dirTo iclRec(file)
   }
}
